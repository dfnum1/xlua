...
static int ll_streamrequire(lua_State *L) 
{
    ...
    else
    {
		...
		if (!lua_istable(L, -1)) {
			lua_pop(L, 2);
			lua_pushliteral(L, "package.searchers not found");
			return lua_error(L);
		}
		...
		for (int i = 1; ; ++i)
		{
			...
			// searcher 返回: loader, extra
			if (lua_isfunction(L, -2))
			{
				// 找到 loader
				lua_pushstring(L, modname);
				lua_pushvalue(L, -3); // extra
				if (lua_pcall(L, 2, 1, 0) != LUA_OK)
				{
					lua_pop(L, 1); // error message
					lua_pop(L, 2); // loader, extra
					continue;
				}
				// loader 返回 chunk 数据
				if (lua_isstring(L, -1))
				{
					chunk_data = lua_tolstring(L, -1, &chunk_len);
					found = 1;
					lua_pop(L, 1); // chunk
					lua_pop(L, 2); // loader, extra
					break;
	...
	if (!found || !chunk_data || chunk_len == 0) {
		lua_pushfstring(L, "module '%s' not found or empty", modname);
		return lua_error(L);
	}
